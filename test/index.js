...
describe('Split text and return Pinyin only', () => {
	it('should split the text into the correct words', done => {
		const list = ['wo', 'de', 'mao', 'xi', 'huan', 'he', 'niu', 'nai']
		split('wo de mao xihuan he niunai', { onlyPinyin: true }).should.deepEqual(list)
		done()
	})
	it('should split the text into the correct words', done => {
		const list = ['wo3', 'de', 'mao1', 'xi3', 'huan1', 'he2', 'niu3', 'nai3']
		split('wo3 de mao1 xi3huan1 he2 niu3nai3', { onlyPinyin: true }).should.deepEqual(list)
		done()
	})
	it('should split the text into the correct words', done => {
		const list = ['wǒ', 'de', 'māo', 'xǐ', 'huān', 'hé', 'niǔ', 'nǎi']
		split('wǒ de māo xǐhuān hé niǔnǎi', { onlyPinyin: true }).should.deepEqual(list)
		done()
	})
})
...
describe('Split spaced text and keep spaces', () => {
	it('should split the text into the correct words', done => {
		const list = [['wo'], ' ', ['de'], ' ', ['mao'], ' ', ['xi'], ['huan'], ' ', ['he'], ' ', ['niu'], ['nai']]
		split('wo de mao xihuan he niunai', {keepSpaces: true}).should.deepEqual(list)
		done()
	})
	it('should split the text into the correct words', done => {
		const list = [['wo3'], ' ', ['de'], ' ', ['mao1'], ' ', ['xi3'], ['huan'], ' ', ['he2'], ' ', ['niu3'], ['nai3']]
		split('wo3 de mao1 xi3huan he2 niu3nai3', {keepSpaces: true}).should.deepEqual(list)
		done()
	})
	it('should split the text into the correct words', done => {
		const list = [['wǒ'], ' ', ['de'], ' ', ['māo'], ' ', ['xǐ'], ['huan'], ' ', ['hé'], ' ', ['niǔ'], ['nǎi']]
		split('wǒ de māo xǐhuan hé niǔnǎi', {keepSpaces: true}).should.deepEqual(list)
		done()
	})
})
